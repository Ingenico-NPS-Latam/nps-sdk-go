...
func CopypStruct(source interface{}, target interface{}) {
	...
	for i := 0; i < s.NumField(); i++ {
	  f := s.Field(i)
	  targetField := t.FieldByName(sTypeOfT.Field(i).Name)

	  if targetField.Kind().String() ==  f.Kind().String() {

	    if f.Kind().String() == "ptr" {

	      if f.Interface() != reflect.Zero(f.Type()).Interface() {
		targetStruct := reflect.New(targetField.Type().Elem())
   		targetField.Set(targetStruct)
		CopypStruct(f.Interface(), targetField.Interface()) 
	      }

	    } else {

		if f.Kind().String() == "slice" {
		  starget := targetField.Type()
		  sliceType := starget.Elem()
		  if f.Len() > 0 {
		  ...
		  for j := 0; j < f.Len(); j++ {
		    itemSource := f.Index(j).Interface()
		    itemTarget := reflect.New(sliceType)
		    CopypStruct(itemSource, itemTarget.Interface()) 
		    targetField.Set(reflect.Append(targetField, itemTarget))
		  }
		  ...
		  if f.Type().String() == "string" {
		    ...
		    if len(f.Interface().(string)) > 0 {
		      ...
		      buffer.WriteString(f.Interface().(string))
	    ...
	    fmt.Printf("CopypStruct [%d] [%s] No existe en target\n", i, sTypeOfT.Field(i).Name)
	  }
